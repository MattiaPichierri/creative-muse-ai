...
def generate_creative_idea(prompt: str, category: str = "general", creativity_level: int = 5) -> dict:
...
async def generate_idea(request: IdeaRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Fehler bei der Ideengenerierung: {str(e)}")
...
async def get_ideas(limit: int = 10, category: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Fehler beim Abrufen der Ideen: {str(e)}")
...
async def rate_idea(idea_id: str, rating: IdeaRating):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Fehler bei der Bewertung: {str(e)}")
...
async def get_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Fehler bei den Statistiken: {str(e)}")
...
if __name__ == "__main__":
    ...
    if not init_simple_db():
        logger.warning("⚠️ Datenbank-Initialisierung fehlgeschlagen, verwende In-Memory-Speicher")
