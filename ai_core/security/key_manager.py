    ...
    def generate_key(
        ...
        try:
            with self.lock:
                ...
                expires_at = (
                    now + timedelta(days=rotation_interval_days)
                ...
                logger.info(
                    f"Schlüssel generiert: {key_id} ({key_type.value}, {algorithm})"
                )
                return key_id
        ...
        try:
            if self.key_rotation_config.get("auto_rotate", True):
                self.rotation_thread = threading.Thread(
                    target=self._auto_rotation_worker, daemon=True, name="KeyRotation"
                )
    ...
    def _check_and_rotate_keys(self):
        ...
        try:
            with self.lock:
                ...
                if keys_to_rotate:
                    logger.info(
                        f"Automatische Rotation: {len(keys_to_rotate)} Schlüssel rotiert"
                    )
    ...
    def cleanup_old_keys(self, max_age_days: int = 365):
        ...
        try:
            with self.lock:
                ...
                for key_id, metadata in self.key_metadata.items():
                    if (
                        metadata.status in [KeyStatus.ARCHIVED, KeyStatus.REVOKED]
