...
class SessionManager:
    ...
    def __init__(
        ...
        self.session_config = (
            config.get("network", {}).get("api", {}).get("authentication", {})
        )
        self.session_timeout_minutes = self.session_config.get(
            "session_timeout_minutes", 30
        )
        self.max_concurrent_sessions = self.session_config.get(
        ...
        try:
            with self.lock:
                ...
                if user_id and self._check_max_sessions(user_id):
                    raise ValueError(
                        f"Maximale Anzahl gleichzeitiger Sessions erreicht: {self.max_concurrent_sessions}"
                    )
                ...
                if session_data:
                    ...
                    encrypted_data = self.crypto_manager.encrypt_string(
                        data_json, "session"
                    )
            ...
            with self.lock:
                ...
                if not session:
                    self._log_security_event(
                        "session_not_found", session_id, ip_address
                    )
                    return None
                ...
                security_flags = self._check_session_security(
    ...
    def get_session_data(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.lock:
                ...
                decrypted_json = self.crypto_manager.decrypt_string(
                    encrypted_data, "session"
                )
                ...
                for session_id in session_ids:
                    if session_id != exclude_session:
                        if self.terminate_session(
                            session_id, "user_sessions_terminated"
                        ):
                            terminated_count += 1
    ...
    def _check_session_security(
        ...
        try:
            ...
            if ip_address and session.ip_address and ip_address != session.ip_address:
                ...
                self._log_security_event(
                    "ip_address_changed", session.session_id, ip_address
                )
            ...
            if user_agent and session.user_agent and user_agent != session.user_agent:
                ...
                self._log_security_event(
                    "user_agent_changed", session.session_id, ip_address
                )
    ...
    def _log_security_event(
        self, event_type: str, session_id: str, ip_address: Optional[str]
    ):
            ...
            try:
                ...
                with self.lock:
                    ...
                    if expired_sessions or old_sessions:
                        logger.info(
                            f"Session-Cleanup: {len(expired_sessions)} abgelaufen, {len(old_sessions)} gelöscht"
                        )
