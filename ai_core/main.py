...
async def require_session(
    ...
    if not session_id:
        raise HTTPException(status_code=401, detail="Gültige Session erforderlich")
    return session_id
...
async def health_check():
    ...
    try:
        ...
        status = {
            "status": "healthy",
            "timestamp": (
                audit_logger.audit_logger.handlers[0].stream.name
                if audit_logger
                else None
            ),
            "components": {
...
async def get_statistics(session_id: str = Depends(require_session)):
    ...
    try:
        stats = {
            "session_manager": (
                session_manager.get_statistics() if session_manager else {}
            ),
...
async def process_websocket_message(
    ...
    try:
        ...
        return {"type": "error", "message": "Nachrichtenverarbeitung fehlgeschlagen"}
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500, content={"error": "Interner Serverfehler", "status_code": 500}
    )
