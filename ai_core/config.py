...
class SecurityConfig:
    ...
    def _load_security_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.warning(f"Sicherheitskonfiguration nicht gefunden: {SECURITY_CONFIG_PATH}")
    ...
    def _setup_logging(self):
        ...
        log_level = self.config.get('development', {}).get('debug', {}).get('log_level', 'INFO')
...
class DatabaseConfig(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///./database/creative_muse.db",
        description="Datenbank-URL"
    )
    
    # Verschlüsselungseinstellungen
    encrypt_database: bool = Field(
        default=True,
        description="Datenbank verschlüsseln"
    )
    
    # Connection Pool
    pool_size: int = Field(
        default=5,
        description="Verbindungspool-Größe"
    )
...
class AIModelConfig(BaseSettings):
    ...
    device: str = Field(
        default="auto",
        description="Gerät für Modell-Inferenz (auto, cpu, cuda)"
    )
    
    max_memory_gb: int = Field(
        default=8,
        description="Maximaler Speicherverbrauch in GB"
    )
    ...
    input_max_length: int = Field(
        default=10000,
        description="Maximale Eingabelänge"
    )
    
    enable_content_filter: bool = Field(
        default=True,
        description="Content-Filter aktivieren"
    )
...
class APIConfig(BaseSettings):
    ...
    use_tls: bool = Field(
        default=True,
        description="TLS verwenden"
    )
    
    cert_file: str = Field(
        default="./security/certificates/server.crt",
        description="TLS-Zertifikat"
    )
    ...
    rate_limit_requests: int = Field(
        default=60,
        description="Anfragen pro Minute"
    )
    ...
    max_concurrent_sessions: int = Field(
        default=1,
        description="Maximale gleichzeitige Sessions"
    )
...
class MonitoringConfig(BaseSettings):
    ...
    metrics_interval_seconds: int = Field(
        default=30,
        description="Metriken-Intervall in Sekunden"
    )
    
    # Alert-Schwellenwerte
    cpu_alert_threshold: int = Field(
        default=80,
        description="CPU-Alert-Schwellenwert in Prozent"
    )
    
    memory_alert_threshold: int = Field(
        default=85,
        description="Speicher-Alert-Schwellenwert in Prozent"
    )
    
    disk_alert_threshold: int = Field(
        default=90,
        description="Festplatten-Alert-Schwellenwert in Prozent"
    )
    
    # Audit-Konfiguration
    audit_level: int = Field(
        default=4,
        description="Audit-Level (1-5)"
    )
